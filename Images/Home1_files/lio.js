...
window['ly_cid'] = "067a9e058db68b69584e726a6255fcb7";
...
(function(d, w) {
  ...
  w.lio = {
    account: {
      id: "067a9e058db68b69584e726a6255fcb7"
    },
    loaded: false,
    ...
    integrationsConfig:{
      allowKrux: false,
      allowPathfora: true,
      customCookieKeys: ["_sp_id.8c88"],
      customDataKeys: [],
      globalPathforaStylesheet: "",
      googleAdsIntegration: [{"web_property":"","segments_dimension":"","user_id_dimension":""}],
    ...
    checkPushIntegrations: function() {
      ...
      if(w.lio.integrations.krux === 0 && w.lio.integrationsConfig.allowKrux && !w.lio.isBlacklisted('krux')){
        var kruxBaseUrl = 'http://beacon.krxd.net/usermatch.gif?';
      ...
      if(w.lio.integrations.adroll === 0 && w.jstag && w.__adroll && !w.lio.isBlacklisted("adroll")){
        ...
        for (var lioKey in w.lio.segmentsCookie) {
          if (!w.lio.segmentsCookie.hasOwnProperty(lioKey)){
  ...
  // get the seerid from cookies
  w.lio._uid = w.lio.getCookie("seerid");

  // if this is the first visit  we may not have a cookie yet, so get it or create it
  if(w.lio._uid == null || w.lio._uid === ''){
    if(w.jstag && typeof w.jstag.getid === 'function'){
      w.lio._uid = w.jstag.getid();
    }
  }
...
if(typeof window.jstag !== 'object'){
    ...
    initObj.cid = "067a9e058db68b69584e726a6255fcb7";
